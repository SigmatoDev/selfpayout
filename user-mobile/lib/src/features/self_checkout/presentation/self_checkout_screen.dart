...
class SelfCheckoutScreen extends ConsumerStatefulWidget {
  const SelfCheckoutScreen({super.key, this.embedded = false});
...
class _SelfCheckoutScreenState extends ConsumerState<SelfCheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final sessionsView = sessionsAsync.when(
      data: (sessions) {
        if (sessions.isEmpty) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _SessionCard(
              ...
              onMarkPaid: session.status == SessionStatus.submitted ? () => _markPaid(session.id) : null,
              onVerify: session.status == SessionStatus.paid ? () => _verify(session.id) : null,
              feedback: _feedback,
    ...
    return Column(
      children: [
        ...
        widget.embedded
            ? SizedBox(height: 320, child: sessionsView)
            : Expanded(child: sessionsView),
      ],
    );
  }
          ...
          children: [
            ...
            if (session.tableNumber != null)
              Text('Table: ${session.tableNumber}'
                  '${session.guestCount != null ? ' • Guests: ${session.guestCount}' : ''}'),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (session.serviceChargePct != null && session.serviceChargePct! > 0)
                    Text('Service charge: ${session.serviceChargePct}%'),
                  ...session.items.map(
                    (item) => Padding(
